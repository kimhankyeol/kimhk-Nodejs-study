...
function getCommentAll(){
  ...
  xhr.onload = function(){
    if(xhr.status === 200){
      ...
      console.log("전체조회 찍히는지 확인중 "+com);
      console.log("로그 확인중 "+com[0].id);
      ...
      com.map(function(c){
        ...
        row.appendChild(td);
        td = document.createElement('td');
        td.textContent = c.user.name;
        console.log("test");
        console.log(c.commenter);
        row.appendChild(td);
        td = document.createElement('td');
        td.textContent = c.comment;
        row.appendChild(td);
        tbody.appendChild(row);
      })
    }
  }
  xhr.open('GET','/comments/all');
  xhr.send();
}
