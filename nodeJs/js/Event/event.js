...
/*
이제는 스트림에서 보았던 on('data')와 on('end') 도 어느정도 감이 올거임
겉으로 이 이벤트를 호출하는 코드는 없지만 내부적으로는 chunk를 전달할때마다 data이벤트를 emit하고 있음
완료되었을 경우에는 end이벤트를 emit한거임

이제 직접 이벤트를 만들수 있으므로 다양한 동작을 직접 구현할수 있음 
웹서버를 구축할때 많이 사용함
지금까지 배운 개념들로만으로도 서버를 만들기에 충분함 
하지만 서버를 운영할떄 코드에 에러가 발생하는것은 치명적이므로 마지막으로 에러를 처리하는 방법에 대해 살펴보겠음
*/